...
export async function GET(req) {
  ...
  const threshold = Number(
    db.prepare("SELECT value FROM settings WHERE key='attendance_threshold'").get()?.value || 80
  );

  // reuse overall logic quickly by calling the same SQL approach
  ...
  const rows = students
    .map((s) => {
      ...
      return { student_id: s.student_id, name: s.name, percent, status: percent < threshold ? "LOW" : "OK" };
    })
